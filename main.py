from tkinter import *
from pandas import *
from random import *

# ---------------------------- global ------------------------------- #
try:
    data = read_csv("words_to_learn.csv")

except:
    data = read_csv("french_words.csv")


to_learn = data.to_dict(orient="records")


current_card = {}
BACKGROUND_COLOR = "#B1DDC6"


# ---------------------------- functions ------------------------------- #

def expose_word():
    canvas.itemconfig(card_title, text="English",fill = "white")
    canvas.itemconfig(card_word, text=current_card["English"],fill = "white")
    canvas.itemconfig(card_background,image = card_back_img )

def next_card():
    global  current_card,expose_timer
    window.after_cancel(expose_timer)
    current_card = choice(to_learn)
    #to change the data in the text as pressing the buttons

    canvas.itemconfig(card_title,text = "French",fill = 'black')
    canvas.itemconfig(card_word, text = current_card["French"],fill = "black")
    canvas.itemconfig(card_background, image=card_front_img)
    expose_timer = window.after(3000, expose_word)


def is_known():
    to_learn.remove(current_card)
    data = DataFrame(to_learn)
    data.to_csv("words_to_learn.csv",index = False)
    next_card()
# ---------------------------- window ------------------------------- #
window = Tk()
window.config(padx=50,pady=50,bg = BACKGROUND_COLOR)
window.title("Flashy")

expose_timer = window.after(1000, expose_word)



# ---------------------------- canvas ------------------------------- #
canvas = Canvas(width= 800,height=520)
card_front_img = PhotoImage(file ="card_front.png")
card_back_img = PhotoImage(file ="card_back.png")

card_background = canvas.create_image(400, 260, image = card_front_img)

card_title = canvas.create_text(400,150,text="Title",font = ("Ariel",40,"italic"))
card_word = canvas.create_text(400,260,text="Word",font = ("Ariel",60,"bold"))

canvas.config(bg = BACKGROUND_COLOR,highlightthickness=0)
canvas.grid(row =0,column=0,columnspan=2)




# ----------------------------  Button ------------------------------- #
wrong_img = PhotoImage(file ="wrong.png")
wrong_btn = Button(image = wrong_img,highlightthickness=0,command=next_card)
wrong_btn.grid(row=1, column=0)

right_img = PhotoImage(file = "right.png")
right_btn = Button(image = right_img,highlightthickness=0,command=is_known)
right_btn.grid(row=1,column=1)


next_card()


window.mainloop()

